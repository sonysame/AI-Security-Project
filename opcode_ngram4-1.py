...
def main():
	...
	with open("_2gram_antivm_binary_feature_selected", 'r') as reader:
		data=reader.read()
		lines=data.split(",\n")[:-1]
	for i in lines:
		tmp = i.replace('"', '').replace(' ', '').replace("]", '').replace("[", '')
		if i not in _2gram_feature_opcode:
			_2gram_feature_opcode.append(tmp)

	_3gram_feature_opcode = []
	with open("_3gram_antivm_binary_feature_selected", 'r') as reader:
		data = reader.read()
		lines = data.split(",\n")[:-1]
	for i in lines:
		tmp = i.replace('"', '').replace(' ', '').replace("]", '').replace("[", '')
		if i not in _3gram_feature_opcode:
			_3gram_feature_opcode.append(tmp)

	_4gram_feature_opcode = []
	with open("_4gram_antivm_binary_feature_selected", 'r') as reader:
		data = reader.read()
		lines = data.split(",\n")[:-1]
	for i in lines:
		tmp = i.replace('"', '').replace(' ', '').replace("]", '').replace("[", '')
		if i not in _4gram_feature_opcode:
	...
	path_file= [ori_path+("/antivm_result"),ori_path+("/original_result")]
	total_file=[os.listdir(path_file[0])[:62],os.listdir(path_file[1])[:62]]

	_2gram_dataset=np.zeros((len(total_file[0])+len(total_file[1]),len(_2gram_feature_opcode)+1))
	...
	_3gram_dataset = np.zeros((len(total_file[0]) + len(total_file[1]),
	...
	_4gram_dataset = np.zeros((len(total_file[0]) + len(total_file[1]) ,
	...
	total_file_test = [os.listdir(path_file[0])[62:], os.listdir(path_file[1])[62:]]

	_2gram_dataset_test = np.zeros((len(total_file_test[0]) + len(total_file_test[1]), len(_2gram_feature_opcode) + 1))
	...
	_3gram_dataset_test = np.zeros(
		(len(total_file_test[0]) + len(total_file_test[1]),
		 len(_3gram_feature_opcode) + 1))
	for category_idx in range(len(total_file_test)):
		for i in tqdm(range(len(total_file_test[category_idx])), mininterval=1):
			target_file = total_file_test[category_idx][i]
